...
Indichiamo con \hl{$\vdash^*$ la chiusura transitiva e riflessiva di $\vdash$}. Essa
non è altro che l'insieme delle nostre \hl{vecchie sequenze di mosse}. Usando
$\vdash^*$ possiamo anche \hl{formalizzare l'accettazione di una stringa} (e la sua
eventuale traduzione) come:

\begin{equation}
  c_0 = \langle q_0, x, Z_0, (\epsilon) \rangle \vdash^*
    c_f = \langle q, \epsilon, \gamma , (z) \rangle
\end{equation}

\begin{nota}
  D'ora in poi indicheremo la chiusura transitiva e riflessiva di una relazione
  $A$ con $A^*$. La chiusura solo transitiva sarà $A^+$.
\end{nota}
...
\begin{itemize}
  ...
  \item Per convenzione la \hl{$\delta (\eta)$ non è definita a partire dagli
    ...
    \[
      \forall q \in F \delta(q, \ldots) = \perp (\eta (q, \ldots) = \perp)
    \]
...
\begin{gather}
  \mhl{ \delta: \; Q \times I \to \mathcal{P}(Q) } \\
  \mhl{
    \delta^\star(q,x) =
    \begin{cases}
...
\begin{equation}
  \mhl{
    \delta: Q \times (I \cup \{\epsilon\}) \times \Gamma \to
      \mathcal{P}_f(Q \times \Gamma^\star)
  }
\end{equation}
...
$\cup$ ma non rispetto a $\cap$ non può esserlo rispetto al complemento}.
