...
con $n!$ foglie, dove $n$ è il numero di elementi}. Assumiamo che \hl{non ci % chktex 40
...
\begin{description}
  \item[Dividi] Scelgo un elemento \hl{\texttt{A[p]}, detto pivot, come punto di
    ...
    essa in modo che tutti quelli di \texttt{A[lo..p - 1]} siano minori del % chktex 8
    pivot}.
  \item[Impera] \hl{Ordina ricorsivamente \texttt{A[lo..p - 1]} e % chktex 8
...
La \hl{parte più importante} dell'algoritmo è \hl{\texttt{Partition()}}. % chktex 36
Studiamo \hl{due implementazioni di questa funzione}: una è attribuita a Lomuto
(\hl{\texttt{PartitionLomuto()}}) mentre l'altra è attribuita al creatore del % chktex 36
quicksort, Hoare (\hl{\texttt{PartitionHoare()}}). % chktex 36
