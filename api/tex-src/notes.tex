...
Anche per \hl{ampliare lo stoccaggio} useremo la \hl{stessa strategia} che
abbiamo usato per le \hl{code}.
...
È importante che \hl{$h_2$ non restituisca mai zero}, sennò la sequenza di
probing degenererebbe.
...
Lo pseudocodice della ricerca del successore è l'algoritmo~\ref{lst:bst-next}.
Anche in questo caso la complessità è \hl{$\mathcal{O}(h)$}, sia nel caso ottimo
che nel pessimo.
...
Anche in questo caso otteniamo \hl{complessità $\mathcal{O}(h)$}. Un algoritmo
che implementa il ragionamento sopra è il seguente.
...
Consideriamo solo i \hl{3 casi del lato sinistro}.
